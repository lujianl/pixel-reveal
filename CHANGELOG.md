...
- **Encoder bitrate retuned against measurements.** The rate was derived from a 0.12 bit/pixel/frame target with a 6 Mbps floor; decoded back and compared with the source, that delivered a mean error of 1.47/255 at 1920×1440, while 0.24 bpp delivers 1.30 and 0.48 bpp only 1.28 — quality saturates around 0.24, which is now the target. The floor is 10 Mbps (the original implementation's flat rate) and the ceiling 20 Mbps, since 2560px would otherwise ask for ~31 Mbps for no visible gain. Note that at low resolutions the encoder's rate control saturates by itself: a 480×360 clip is byte-identical at 6, 10 and 20 Mbps.
- **The preview showed blur that the export did not.** A preview renders at 720px while the export renders at up to 2560px, and Melt's blur is an absolute 20px radius — so the preview's blur was ~3.6× stronger in block terms. For a phone photo (export block size 80) the export carried no blur at all while the preview blurred 33% of its pixels, which made the preview look wrong next to a correct export. Effects now receive `outputBlockSize` and `renderScale` on their context, `createPreview()` accepts `outputMaxDimension` + `sourceSize`, and Melt decides its radius in the export's block units. Measured: export 2560/no blur ↔ preview no blur, export 1280/blurred ↔ preview blurred, export 480 ↔ preview identical.
